...
TEST_F(TestGraphAdapter, BasicArrayIteration)
{
    ...
    ASSERT_ANY_THROW( adapter.getNumber() );
...
TEST_F(TestGraphAdapter, BasicObjectIteration)
{
    ...
    valijson::graph::GraphNode::Object obj;
    for (unsigned int i = 0; i < numElements; i++) {
        const std::string name(boost::lexical_cast<std::string>(i));
        obj[name] = boost::make_shared<valijson::graph::GraphNode>(double(i));

    }

    boost::shared_ptr<valijson::graph::GraphNode> graphNode =
            boost::make_shared<valijson::graph::GraphNode>(obj);
    valijson::adapters::JsonGraphAdapter adapter(graphNode);
    ...
    unsigned int expectedValue = 0;
    BOOST_FOREACH( const valijson::adapters::JsonGraphAdapter::ObjectMember member, adapter.getObject() ) {
